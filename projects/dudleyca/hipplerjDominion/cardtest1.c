...
int main () {
    ...
    cardEffect(adventurer, c1, c2, c3, &state, handpos, &bonus);
    
    printf("\nPlayer0 gains 2 cards\n");
    assertTrue(state.handCount[player0],origState.handCount[player0]+2);
    ...
    for (i = 0; i < origState.handCount[player0]; i++) {
        card = origState.hand[player0][i];
        if (card == copper || card == silver || card == gold) {
            origTcount++;
        }
    }
    ...
    assertTrue(origTcount+2,tCount);
