...
/***********************************************************
 * REFACTORED functions from cardEffect: smithy_Card, adventure_Card, village_Card, councilRoom_Card, steward_Card
 * **********************************************************/ 
...
void adventure_Card(int currPlayer, int tempHand [], int z, struct gameState *state){
	...
	while(t_Drawn < 2) {
		...
		drawCard(currPlayer, state);
		//top card of hand = most recently drawn card
		c_Drawn = state->hand[currPlayer][state->handCount[currPlayer]-1];
	...
	//loop to discard all cards in play that have been drawn
	while(z - 1 >= 0) {
...
void village_Card(int currPlayer, int handPos, struct gameState *state){
	//+1 Card
	drawCard(currPlayer, state);
...
void councilRoom_Card(int currPlayer, int handPos, struct gameState *state){
	...
	//Loop for each player drawing a card
	...
	//the played card is put in the played/discarded card pile
	discardCard(handPos, currPlayer, state, 0);
}

void steward_Card(int currPlayer, int handPos, int c1, int c2,int c3, struct gameState *state){
	...
	//else trash 2 cards from the hand
	else {
...
{
	case adventurer:
	 	adventure_Card(currentPlayer, temphand, z, state);
      		return 0;
			
	case council_room:
		councilRoom_Card(currentPlayer, handPos, state);
		return 0;
	...
	case village:
		village_Card(currentPlayer, handPos, state);
		return 0;
